  ...
  factory: (require) => {
    ...
    const plugin = {
      ...
      apply(ctx) {
        ...
        // 把开关选择持久化到 localStorage，刷新后保持；存储值优先于内置默认值。
        ...
        function ensureStyle() {
          ...
          const css = [
            ...
            // 收起态禁用 transition，否则 data-on 切换时的 background 过渡会卡滞在 hover 色上，
            ...
            // 收起态同一图标、颜色区分开关。
            // 开启用品牌状态色图标 + 浅底，关闭用常规前景色，保证窄 rail 下也能一眼看出开关状态。
          ...
          return React.createElement(
            'button',
            {
              ...
              onClick: function () {
                ...
                if (next) { start(); expandAll() } else { stop(); collapseAll() }
              },
            },
            React.createElement(
              'div',
